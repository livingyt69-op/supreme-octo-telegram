...
export default {
  data: new SlashCommandBuilder()
    .setName('removexp')
    .setDescription('Remove XP from a user account.')
  ...
  async execute(interaction) {
    ...
    if (!user) return interaction.reply({ embeds: [createEmbed({ title: 'Remove XP', description: 'User does not have an account.' })], ephemeral: true });

    const xp = Math.max(0, user.xp - amount);
    const level = calculateLevel(xp);
    updateUser(interaction.client.db, target.id, { xp, level });
    logEvent(interaction.client.db, 'Remove XP', target.id, `Removed ${amount} XP by ${interaction.user.id}`);
    await interaction.reply({ embeds: [createEmbed({ title: 'Remove XP', description: `${amount} XP removed from ${target.tag}. New level: ${level}.` })], ephemeral: true });
  },
};
